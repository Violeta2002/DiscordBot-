#Copyright 2021, 2021 Comsa Violeta 
...
@bot.command(name='play', help='To play a song you need the URL off Youtube ')
async def play(ctx,url):
    if not ctx.message.author.name=="Violeta" :
         await ctx.send('You have to be the admin')
         return
    try :
...
@bot.command(name='pause', help='It pauses the song at a certain moment')
async def pause(ctx):
...
@bot.command(name='resume', help='Resumes the song from the moment it stopped playing')
async def resume(ctx):
    voice_client = ctx.message.guild.voice_client
    if voice_client.is_paused():
        await voice_client.resume()
    else:
        await ctx.send("The bot was not playing anything. Use the *play* command to start listening to music ")
   
@bot.command(name='scram', help='To make the bot leave the channel')
async def scram(ctx):
    voice_client = ctx.message.guild.voice_client
...
@bot.command(name='stop', help='Stops the song that is playing at that moment')
async def stop(ctx):
    voice_client = ctx.message.guild.voice_client
    if voice_client.is_playing():
    ...
    for guild in bot.guilds:
        for channel in guild.text_channels :
            if str(channel) == "general" :
                await channel.send('I am alive now!')
